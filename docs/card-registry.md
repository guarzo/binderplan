...
**Frozen on merge.** IDs stayed provisional for as long as this registry lived on an unmerged branch, and were renumbered whenever an error surfaced — including three misidentified species caught in review, whose IDs were corrected rather than left mismatched. The never-rewrite rule binds from the moment the registry lands on `main` and the ledger can cite it, because that is when references start existing to break. Before then the rule protects nothing. Every other column stays correctable forever — filling in a `set`, correcting a `species`, upgrading a `confidence` are all normal and expected.
...
| marowak-01 | Marowak | ガラガラ | JP |  |  | uncertain | intimidation_1.webp 2026-08-01 | delta species print, distinct from cubone-02 (the カラカラ), number illegible after crop attempt |
...
| noibat-01 | Noibat | オンバット | JP |  |  | uncertain | calm_nature_1.webp 2026-08-01 | set/number code visible but digits ambiguous under magnification, left blank rather than guess |
