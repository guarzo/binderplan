...
Use `scripts/manage-card-images.py` for card-image review and approval. Do not add photographed spreads as inline markup to the reconstructed binder pages. For unresolved printings, non-exact images, and owner-directed digital placement, use the [generated owner validation checklist](docs/card-validation-checklist.md); regenerate it with `python scripts/validation_checklist.py --write` after reviewed records change. The [registry worklist](docs/registry-confirmation.md) serves a different purpose: physical checks for registry confidence and duplicate-printing candidates.
